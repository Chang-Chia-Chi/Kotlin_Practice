...
class ConfigValidatorTest {
    ...
    private val config = mock<FrameworkConfig>()
    private val workerConfig = mock<FrameworkConfig.WorkerConfig>()
    private val leaderConfig = mock<FrameworkConfig.LeaderElectionConfig>()
    private val startupEvent = mock<StartupEvent>()

    private fun validator(poolMaxSize: Int) = ConfigValidator(config, poolMaxSize)

    private fun validDefaults() {
        whenever(config.worker()).thenReturn(workerConfig)
        whenever(config.leaderElection()).thenReturn(leaderConfig)
