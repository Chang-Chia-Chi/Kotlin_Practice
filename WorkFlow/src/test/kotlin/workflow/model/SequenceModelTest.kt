...
class SequenceModelTest {
    ...
    // ── Spec item 7: Unreachable activity (isolated cycle) rejected ──────

    @Test
    fun `isolated cycle unreachable from any root is rejected at build time`() {
        // a is the sole root (indegree 0). c→d→c is an isolated cycle with no
        // incoming edges from the reachable subgraph, so c and d are unreachable.
        assertFailsWith<IllegalArgumentException> {
            WorkflowDefinition(
                activities = mapOf(
                    "a" to ActivityDefinition("a", "a.h"),
                    "c" to ActivityDefinition("c", "c.h", successors = listOf(Edge("d"))),
                    "d" to ActivityDefinition("d", "d.h", successors = listOf(Edge("c"))),
                ),
            )
        }
    }
    ...
    @Test
    fun `standalone activity with no edges becomes its own root under auto-detect`() {
        val def = WorkflowDefinition(
            activities = mapOf(
                "a" to ActivityDefinition("a", "a.h"),
                "standalone" to ActivityDefinition("standalone", "standalone.h"),
            ),
        )
        assertEquals(setOf("a", "standalone"), def.starts.toSet())
    }
