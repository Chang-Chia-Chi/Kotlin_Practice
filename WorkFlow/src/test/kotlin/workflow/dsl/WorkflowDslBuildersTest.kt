...
class WorkflowDslBuildersTest {
    ...
    fun `linear workflow builds correctly`() {
        ...
        assertEquals(listOf("step-1"), def.starts)
    ...
    fun `migrated dispatchWorkflow builds and scatter batchToken resolves from scatter`() {
        ...
        assertEquals(listOf("scatter"), def.starts)
    ...
    @Test
    fun `auto-detects single root from indegree-0 set`() {
        val def = workflow {
            activity("a") { transition("a.h"); next("b") }
            activity("b") { transition("b.h") }
        }
        assertEquals(listOf("a"), def.starts)
    }

    @Test
    fun `auto-detects multiple roots when graph has parallel entry points`() {
        val def = workflow {
            activity("ingest-a") { transition("ia.h"); next("merge") }
            activity("ingest-b") { transition("ib.h"); next("merge") }
            activity("merge") { transition("m.h") }
        }
        assertEquals(setOf("ingest-a", "ingest-b"), def.starts.toSet())
    }

    @Test
    fun `explicit start on non-root activity is rejected`() {
        assertFailsWith<IllegalArgumentException> {
            com.workflow.workflow.model.WorkflowDefinition(
                activities = mapOf(
                    "a" to com.workflow.workflow.model.ActivityDefinition("a", "a.h", successors = listOf(Edge("b"))),
                    "b" to com.workflow.workflow.model.ActivityDefinition("b", "b.h"),
                ),
                start = "b",
            )
        }
