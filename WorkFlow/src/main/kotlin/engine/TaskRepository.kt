...
class TaskRepository(private val jdbi: Jdbi) {
    ...
    suspend fun claimNext(workerId: String, limit: Int): List<Task> =
    ...
    suspend fun updateStatus(id: String, newStatus: TaskStatus, resultJson: String? = null) {
    ...
    fun updateStatusWithHandle(handle: Handle, id: String, newStatus: TaskStatus, resultJson: String? = null) {
        if (newStatus.isTerminal) {
            ...
            handle.createUpdate("UPDATE task SET status = :status, result = :result WHERE id = :id")
                .bind("id", id)
                .bind("status", newStatus.name)
                .bind("result", resultJson)
                .execute()
    ...
    fun insertBatchWithHandle(handle: Handle, tasks: List<Task>) {
        ...
        for (task in tasks) {
            ...
            batch.bind("retryCount", task.retryCount)
    ...
    private fun bindNullableTimestamp(stmt: org.jdbi.v3.core.statement.SqlStatement<*>, name: String, value: Instant?) {
        if (value != null) {
