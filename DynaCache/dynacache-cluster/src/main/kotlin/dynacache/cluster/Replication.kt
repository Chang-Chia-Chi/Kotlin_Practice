 ...
 *
 * Every stored value carries a version, and the [store] holds the pair (T66): a write bumps the
 * key's version with the node's next dot (C2) as it applies locally, then ships the entry it
 * logged, as the engine codec's bytes (ADR 0003), plus that version to the key's other
 * replicas; a read runs locally and on R-1 replicas and answers with the version that
 * dominates (C4). Plan 2.5: every fan-out goes to at most N-1 nodes and waits at most
 * [deadline]. A write whose replica is dead goes to the next healthy node on the ring instead,
 * which keeps it as a hint in [hints] and hands it back when the replica returns (spec 2.4
 * sloppy quorum, spec 5.1 step 7). A read that finds a replica behind the winning version has
 * the winner push its value there afterwards (spec 5.2 step 5): the value crosses as the
 * engine encodes it, and the store decides spec 5.3 on arrival; a replica whose version is
 * concurrent with the winner's is not pushed to.
 *
 * @param engine the node's engine, for what has no key: the store runs every keyed command.
 * @param store the node's (value, version) pairs, over the same engine.
...
class Replication(
    ...
    /** The version this node holds for [key], null when it never stored one: the store's answer. */
    fun version(key: Key): Dvv? = store.version(key)
    ...
    /**
     * Spec 5.1 steps 4 to 6 and 8. The store bumps the version and runs the write as one step,
     * so two writes racing on one key chain instead of forking. What ships is what the log
     * holds (ADR 0003): [whatChanged] decides it, so a write the engine refused, or a
     * conditional `SET` that did not apply, replicates nothing and keeps the reply it got, and
     * the coordinator's instant settles a `SET`'s TTL into the deadline every replica applies
     * (spec 5.4).
     */
    private suspend fun write(command: Command.Keyed): Reply {
        val (reply, dvv) = store.write(command).await()
    ...
    private suspend fun read(command: Command.Keyed): Reply {
        val mine = store.read(command).await()
    ...
    /**
     * Ships this node's value under [key] with its version to [targets], the pair read as one
     * from the store. No value under a version is a tombstone. A key the store holds no
     * version for has nothing to push.
     */
    private suspend fun push(key: Key, targets: Collection<NodeId>) {
        val held = store.held(key).await() ?: return
        val body = Version.newBuilder().setKey(ByteString.copyFrom(key.bytes)).setDvv(ByteString.copyFrom(held.dvv.encode()))
            .setValue(held.value?.let { ByteString.copyFrom(encodeValue(it)) } ?: ByteString.EMPTY)
    ...
    /**
     * The receiving end of a repair: the store decides spec 5.3 for the pair. A tombstone
     * arrives as no value; bytes that do not decode are ignored whole, version included.
     */
    private suspend fun installValue(request: Version) {
        ...
        val value = if (request.value.isEmpty) null else (runCatching { decodeValue(request.value.toByteArray(), Random(remote.dot.counter)) }.getOrNull() ?: return)
        store.install(key, value, expiresAt, remote).await()
    }
    ...
    /**
     * The replica's half of spec 5.1 step 5: the store decides spec 5.3 for the entry and its
     * version. Bytes or a version this node cannot read are not acked, so the coordinator
     * counts this node as silent rather than as agreeing. A write this node only stands in for
     * (`hint_for` set) is kept whole as a hint instead of applied (C5), and acked toward W all
     * the same.
     *
     * The bytes are the entry the coordinator logged, and the replica applies what they decode
    ...
    private suspend fun replicate(from: NodeId, request: Replicate) {
        val decoded = runCatching { CommandCodec.unframe(request.command.toByteArray()) }.getOrNull() ?: return
        val commands = decoded.map { it as? Command.Keyed ?: return }.ifEmpty { return }
        val remote = runCatching { Dvv.decode(request.dvv.toByteArray()) }.getOrNull() ?: return
        ...
        store.apply(commands, remote).await()
        send(from, Envelope.newBuilder().setReplicateAck(ReplicateAck.newBuilder().setId(request.id)))
    }

    /** The replica's half of spec 5.2 step 2: run the read here and answer with the version it read under. */
    private suspend fun answer(from: NodeId, request: Read) {
        ...
        val (reply, dvv) = store.read(command).await()
        val version = dvv?.let { ByteString.copyFrom(it.encode()) } ?: ByteString.EMPTY
        send(from, Envelope.newBuilder().setReadReply(ReadReply.newBuilder().setId(request.id).setReply(ReplyWire.encode(reply)).setDvv(version)))
    }
