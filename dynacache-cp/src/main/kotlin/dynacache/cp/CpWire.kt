...
object CpWire {
    ...
    // T39 wraps the operation in a time-stamped envelope; this is the one pair of functions that
    // has to learn about it, because every other encoding here is below the log entry.
    private fun encodeOperation(operation: Any?): ByteArray = bytes {
        when (operation) {
            is Command.Cp -> writeCommand(operation)
    ...
    private fun decodeOperation(encoded: ByteArray): Any = read(encoded) {
        val tag = readByte().toInt()
        if (tag == OP_INTERNAL) Internal else readCommandBody(tag)
    }
    ...
    private fun DataOutputStream.writeCommand(command: Command.Cp) {
        when (command) {
            is Command.Cp.LongSet -> tagged(CMD_SET, command.key) { writeLong(command.value) }
    ...
    private fun DataInputStream.readCommandBody(tag: Int): Command.Cp {
        ...
        return when (tag) {
            CMD_SET -> Command.Cp.LongSet(key, readLong())
