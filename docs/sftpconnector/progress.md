...
- Two ArchUnit rules, and a spec sentence in 3.1 plus D48 saying what they enforce.
...
- **`config` and `error` sit beside the layer stack, not in it** (spec 3.1, D48). `error` owes
...
declared instead: spec 3.1 says `config` may name the checks and nothing else beneath it, D48 says
