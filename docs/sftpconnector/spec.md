...
`config` and `error` sit beside the stack rather than in it, and each has one rule. `error`
depends on nothing: every layer raises its classes, so anything it imported back would come out
again in the signature of what a caller catches. `config` refers downward exactly once - to the
readiness check types `polling` configures, because choosing which of them runs is what that
block is for - and otherwise names only its own types, the readiness ones, and `error`'s. That
one reference is the only downward reference the layering permits: an overwrite policy, a retry
class, a post-action are all things the DSL configures, and each of them is spelled in `config`
itself rather than reached for in the layer that acts on it. The architecture test fails the
build on a second one (D48).
...
**The forced tier closes the socket before it disconnects the session** (D47, measured against
