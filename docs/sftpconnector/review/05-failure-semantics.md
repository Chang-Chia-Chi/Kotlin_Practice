...
By the ticket owner (a Fable 5.1 subagent, T17 lens 5 fixes), on `misc/ai_gen` after
`109c1b3`. Each fix is its own commit with the test that found it, run red before the fix; no
earlier test's assertion was changed, and the two server-side rename tests whose *setup* changed
are declared under H2. The reviewer's text above is untouched.

| Finding | Disposition |
|---|---|
| **H1** breaker opens on slow successes | **Fixed**, `1b1acc7`. The slow-call rule is switched off with the longest threshold the library takes rather than tuned to `transferTimeout`: the span the breaker measures includes the wait for a transfer permit, which spec 9 bounds only by the transfers ahead of it, so no finite knob makes "a success is never a failure" true. The call's duration is measured on the injected `Clock`, in nanoseconds because `resilience4j-kotlin` 2.4.0's wrapper passes `NANOSECONDS` to `onResult` whatever unit the config declares (read off the bytecode; a millisecond timestamp measured every call at microseconds and hid the defect). Test: `RetrySemanticsTest.a success slower than the library's own minute is not held against the server`, on virtual time - two 61 s successes opened a window of two before the fix |
| **H2** `REPLACE` retry adopts a same-size target | **Fixed**, `d0e81bf`; **D46**. The discriminator is the source's size *and* modification time as listed; a rename keeps both, SFTP reports mtime in whole seconds (`JschTransport.describe`), and both readings compared are the server's own stat, so the listing-to-rename skew the readiness check has to allow for (T15) does not enter. The retry looks at the source first: the listed file still there is a rename that did not land, whatever look-alike the target holds; a source that *changed* reads as landed, so a new same-name upload during the backoff stays in the inbox instead of being moved unprocessed - the coincidences left (a same-second look-alike, an identical re-upload) cost a duplicate, never a loss, which is the at-least-once contract. `rename(from, to, overwrite, listed: RemoteFile?)` replaces the bare size; the source passes its `RemoteFile`, a caller without one costs the same single stat as before. The fake carries a per-file mtime and moves the entry whole on rename; `listed(path)` reads it back. Setup changed, assertions untouched (C5's shape): `PartitionMatrixTest.P3` and `ResilienceAgainstServerTest.I11_..` list the file through the client before arming their faults - a stat inside the try would have been the request the reset hit. Tests: `RetrySemanticsTest.I15_under REPLACE a file already at the target with the source's size is not taken for the landed one` and `..a look-alike at the target does not outweigh the listed file still at the source`; `AdversaryTest.I15_`, `P3` green. Spec 6.1, 8.3, I11 amended |
| **M1** a long read is retried and counted | **Recorded**: D28 amended in the decision log with the reviewer's proposed wording as the maintainer's decision, and spec 10.1's row reads "poisons on a short read, answered and kept on a long one". The code (`StagingArea`'s check and `IncompleteTransfer`'s disposition, one flag) is the maintainer's to land with the test the reviewer wrote out, since it touches T2's `FailureModelTest` and is outside a lens fix. Until then the spec leads the code on that row |
| **M2** `watch` swallows a collector's cancellation | **Fixed**, `0c9adc9`. Only the receive is inside the catch: a channel closed by a cancellation is the connector stopping its watchers, closed by a failure is a tick no later tick could survive, and whatever `emit` throws passes through untouched. Test: `SftpWatchTest.a timeout the collector lets out of its own block ends the watch with that timeout, not normally` |
| **M3** probe relabels wire failures `Fatal` | **Handled by lens 4** (`73782e3`, with lens 4 H1) |
| **M4** last-try lost reply not looked into | **Fixed**, `5c83999`, the look only. `SftpClient.rename` catches a wire failure out of the retry loop and makes the look the next retry would have made, once, on a fresh session behind the breaker (`Resilience.once`, so an open breaker refuses it and the lost reply is reported as it was, the look's failure suppressed under it). Test: `RetrySemanticsTest.a rename whose reply is lost on the last permitted try is looked into before it is reported`. **Not changed:** the source's WARN at `answering` ("still where it was") - the brief keeps `SftpSource` edits to the `watch` catch; after this fix the line is true whenever the look ran, and unverified only when the look itself failed, which is the residual. The I15 wording is the maintainer's (T16) |
| **L1** cancelled acquire evicts a healthy session as poisoned | **Recorded**; owner: the cancellation-ladder batch (`SftpPool.kt` is theirs) |
| **L2** `openForTheShelf` swallows an `Error` | **Recorded**; owner: the maintainer, with the ladder batch since it is `SftpPool.kt` |
| **L3** a `list` filter's exception reported as `ServerFailure` | **Recorded**; owner: the maintainer (`JschTransport.list` is the ladder batch's region this ticket; the mapper row belongs with L4) |
| **L4** `SSH_FX_NO_CONNECTION` and `SSH_FX_CONNECTION_LOST` map to `ServerFailure` | **Recorded**; owner: the maintainer (`JschErrorMapper`, two rows to `SessionLost`) |
| **L5** the breaker cannot judge before twenty tries | **Recorded** as consistent with spec 9; owner: the maintainer, a documentation note for the hourly pipeline |
| **L6** a `CircuitOpen` inside `consume` is counted as a nack | **Owned by ticket 20** |

**Verdict on the lens.** Two Highs fixed, two Mediums fixed, one Medium recorded as a decision,
one already fixed by lens 4; six Lows recorded with owners. `spec.md` gained D46 and an amended
D28; `SftpClient.rename`'s signature changed for D46 and every caller in the reactor was moved.
