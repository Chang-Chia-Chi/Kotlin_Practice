...
digest (D48): a mapping row may name an algorithm with `digest: <algo>`, but only to state which
...
| `digest: <algo>` | with `field: DIGEST`, asks for a specific algorithm; the row is missing when the transfer's digest is another (D48), and rule 26 rejects the mismatch at boot |
...
| 26 | A mapping row's `digest: <algo>` names the algorithm its route computes, `digest` or the process default (D48) |
