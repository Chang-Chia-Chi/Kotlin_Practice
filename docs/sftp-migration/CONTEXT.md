...
Before any write/swap, the migration script must confirm NAS2 is available by **reading a sentinel file** placed on the real NAS2 (`head -c1 /mnt/nas2/.nas2_sentinel || exit`). Prevents the NFS-shadow footgun (unmounted → writes land on local root disk). Chosen over `mountpoint -q` because it also catches **stale** mounts (ESTALE), and because it is **trivially testable** — tests `touch`/remove the sentinel in a local dir to simulate mounted/unmounted, with no mount privileges or real NFS. The same sentinel can feed infra's NAS2 health monitoring.
