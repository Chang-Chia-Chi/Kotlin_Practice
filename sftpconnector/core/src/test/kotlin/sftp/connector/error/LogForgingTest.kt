...
class LogForgingTest {
    ...
    fun `an unmapped server message is logged on one line`() {
        val logged = capturingStandardError {
            JschErrorMapper(SimpleMeterRegistry()).translating(Attempt("sftp.example:22", "list", "/drop")) {
        ...
        assertTrue(logged.lines().none { it.trimStart().startsWith("WARN the connector is fine") }, logged)
    }
