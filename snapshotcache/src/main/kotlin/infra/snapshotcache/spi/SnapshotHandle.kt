...
/** One shared Cleaner for every handle (spec 6.3): a single daemon thread is plenty for a bug-path backstop. */
 ...
 * `java.sql` types (plan 2.2 rule 4, D28). `core` constructs it from the lease's
 ...
 * cleanup with `orphaned = true` - a bug signal, never a normal path (spec 6.3). The
...
internal class SnapshotHandle(
     ...
     * `onRelease` fires exactly once however the races fall (spec 6.3 rule 3, I6).
     */
