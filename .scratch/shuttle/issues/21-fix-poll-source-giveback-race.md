...
- [x] A pipeline's fetch and ack act on the hand-over that launched it, never on a later hand-over of the same path (key the map by the hand-over, or refuse a second entry for a path still in flight, or fail the old fetch when its own entry is gone; choose the shape that keeps the connector's slot accounting exact)
- [x] The existing `SftpPollSourceTest` cases stay green, in particular the D40 give-back and the run-ends-gives-back-everything cases
- [x] The `ponytail:` comment and progress 13's deviation 4 are corrected to what is now true
