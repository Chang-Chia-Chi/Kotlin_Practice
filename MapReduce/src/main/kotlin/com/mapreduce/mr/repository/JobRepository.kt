 ...
 */
@ApplicationScoped
class JobRepository(private val jdbi: Jdbi) {
    ...
    fun findJobsByStatus(status: JobStatus): List<Job> =
        jdbi.withHandle<List<Job>, Exception> { h ->
            h.createQuery("SELECT * FROM mr_job WHERE status = :status")
    ...
    fun setResultMetadata(jobId: String, metadata: String) {
    ...
    /**
     * Atomically: persist map outputs, mark task COMPLETED, increment completed_tasks.
     * All in one transaction for correctness (design doc S5.5 Phase 2, step 4).
     */
    fun completeMapTask(taskId: String, jobId: String, outputs: List<String>) {
        jdbi.useTransaction<Exception> { h ->
    ...
    fun completeReduceTask(taskId: String, jobId: String, resultMetadata: String) {
        jdbi.useTransaction<Exception> { h ->
            h.createUpdate(
                "UPDATE task SET status = 'COMPLETED', completed_at = CURRENT_TIMESTAMP WHERE task_id = :taskId"
            ).bind("taskId", taskId).execute()
    ...
    fun insertReduceTask(jobId: String, jobType: String, maxRetries: Int, queue: String) {
        jdbi.useHandle<Exception> { h ->
            h.createUpdate(
                """
                INSERT INTO task (task_id, handler, queue, payload, status, priority,
    ...
    fun updateFailedTasks(jobId: String, failedCount: Int) {
        jdbi.useHandle<Exception> { h ->
            h.createUpdate(
     ...
     */
    fun streamOutputs(jobId: String): Flow<String> = flow {
