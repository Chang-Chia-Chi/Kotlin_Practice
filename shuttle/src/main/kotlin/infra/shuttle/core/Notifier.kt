 ...
 * in `finally`, on every exit path (I4). A [wake] follows every transaction that creates rows; the
 * sweep every `sweepEvery` is the guarantee. Cancelling [run] leaves rows PENDING and the set empty.
            ...
            for (delivery in due) {
                ...
                launch {
                    try {
                        ...
                        inFlight -= delivery.id
                        permits.release()
                    }
                }
            }
            refreshGauges()
            if (due.size < config.batch) withTimeoutOrNull(config.sweepEvery) { wake.receive() }
        }
    }
